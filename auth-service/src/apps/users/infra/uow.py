...
class UserUnitOfWork(DatabaseUnitOfWork):
    def __init__(self, session: AsyncSession):
        ...
        self._user_repository = UserRepository(model=User, session=session)
        self._user_role_repository = RoleRepository(model=Role, session=session)
        self._user_session_repository = UserSessionRepository(model=UserSession, session=session)
